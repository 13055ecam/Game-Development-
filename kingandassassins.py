#!/usr/bin/env python3
# kingandassassins.py
# Author: Alexandre Lenaerts
# Version: May 13, 2016
from math import sqrt
from knightmodule import knightmove, kingmove, king_localisation, pathinder_look, pathfinder
import argparse
import json
import random
import socket
...
class KingAndAssassinsClient(game.GameClient):
    ...
    def _nextmove(self, state):
        ...
        kingloc= kingcoord(state)
        if state['card'] is None:
##      3 assassins are randomly selected
            self._assassins=random.sample(POPULATION,3)
            return json.dumps({'assassins': self._assassins}, separators=(',', ':'))
##
##      If we are assassins : the player moves a assassin on the right when It's his turn
        if self._playernb == 1:
            if self._round==6:
                for horizontal_lign in range(10) :
                    for vertical_lign in range(10):
                        if state['people'][horizontal_lign][vertical_lign] == self._random_assassins[0]:
                            return json.dumps({'actions': [('move',(horizontal_lign,vertical_lign,'E'))]}, separators=(',', ':'))
        else:
            #knights follow the king 
            if self._round  != 4:
                while state['card'][1] > 0:
                    if len(pathfinder(state, kingcoord)) > 0:
                            step.append(pathfinder(state, kingcoord))
                            state['card'][1] -= 1
                    for i in range(10):
                        for j in range(10):
                            knightactions = knightmovelist(kingcoord, (i, j), state['card'], state, POPULATION)
                            if state['people'][i][j] == 'knight' and len(knightactions[0])>0:
                                for i in range(len(knightactions[0])):
                                    knightmovelist.append(knightactions[0][i])
                                    state['card'][1] = knightactions[1]
                        step.append(kingaction(kingloc, 0, state['card'], 0))
                    return json.dumps({'actions': action}, separators=(',', ':'))
                
                    return json.dumps({'actions': knightmove((9, 9), (8, 9), state['card'], 0)[0]}, separators=(',', ':'))
